<template>
  <div>
      <breadcrumb :data="['历史作业']"></breadcrumb>
    <div>
...
export default {
  ...
  created() {
    ...
    $httpc
      .get(`/user/${u_id}/course/${c_id}/test?type=1&isCurrent=false`)
      .then(res => {
        this.workList = res;
      });
  },
  methods:{
    ...
    workDetail(row){
      this.$router.push(`last_work/${row.test_id}?action=watch&isSubmit=${row.submit_time==0?'no':'yes'}`);
    }
  }
};
</script>
