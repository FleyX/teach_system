...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      $http.get(`question_answer/${this.data.qa_id}/detail?time=${this.lastUpdateTime}&type=${this.type}`).then(res => {
        if (res.length > 0) {
      ...
      //获取对方头像
      $http.get(`user/${this.type == 'start' ? this.data.reply_u_id : this.data.start_u_id}`).then(res => {
    ...
    sendData() {
      $http.post(`/question_answer/${this.data.qa_id}/question_answer_detail`, this.form).then(res => {
        this.form.content = '';
      })
    },
    close() {
