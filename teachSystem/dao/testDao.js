...
class testDao {
        ...
        if (res == null) {
            ...
            for (let i = 1; i <= 7; i++) {
                ...
                let value = await Mysqlhelper.single(`
                    select count(*) from student_test where submit_time<? and submit_time>?`, lastDate.getTime(), tempDate.getTime());
                res.data.push({
                    key,
                    value
                });
    ...
    static async getTest(u_id, c_id, param) {
        ...
        if (param.isCurrent === 'true') {
            let str;
            if (param.type == '1')
                str = `start_time<${now} and end_time>${now} and submit_time =0`;
            else if (param.type == '2')
                str = `end_time>${now} and submit_time=0`;
            else
                throw ErrorHelper.Error400('请求参数错误' + param.type);
        ...
        if (res == null || type == 'new') {
            ...
            if (qg_id != null) {
                ...
                questionKnowledge.forEach(item => {
                    ...
                    if (questionKnowledgeObj[item.ql_id] == null)
                        questionKnowledgeObj[item.ql_id] = [];
                ...
                res.score.forEach(item => {
                    ...
                    for (let key in sum) {
                        ...
                        if (sum[key] == 1)
                            question.rightCount++;
                        if (questionKnowledgeObj[key] == undefined)
                            return;
                        questionKnowledgeObj[key].forEach(item1 => knowledgeObj[item1].count++);
                        if (sum[key] == 1)
                            questionKnowledgeObj[key].forEach(item1 => knowledgeObj[item1].rightCount++);
                    }
                })
    ...
    static async getTestDetail(u_id, c_id, test_id) {
        ...
        if (data.qg_id == null) {
            ...
            let temp = []; //记录未达到5题的难度
            for (let i = 1; i <= 5; i++) {
                if (studentInfo['difficulty_' + i] < 5)
                    temp.push(i);
            }
            if (temp.length != 0) {
                //均分每种难度占比
    ...
    static async addOneKnowledgeTest(u_id, c_id, param) {
        ...
        if (count > 0)
            throw ErrorHelper.Error403("您还有未完成练习，请先完成");
        ...
        let test_id = await Mysqlhelper.insert(`
                ...
                `, c_id, 3, JSON.stringify(param), Date.now(), Number.MAX_SAFE_INTEGER, 0, param.kp_name + '-专项练习', 1, '');
    ...
    static async addOne(param) {
        ...
        if (param.end_time != '')
            param.end_time = Math.ceil(new Date(param.end_time).getTime() / 1000) * 1000;
        else
            param.end_time = param.start_time + parseInt(param.work_time) * 60 * 1000;
        let res;
        if (param.get_question_type == '1') {
            ...
            res = await Mysqlhelper.insert(`
                ...
                `, param.c_id, param.questionData.qg_id, param.questionData.score, param.t_type, param.start_time, param.end_time, param.work_time, param.test_name, 0, param.demand);
            ...
            res = await Mysqlhelper.insert(`
                ...
                `, param.c_id, 1, param.questionData.allScore, JSON.stringify(param.questionData), param.start_time, param.end_time, param.work_time, param.test_name, 1, param.demand);
            ...
            res = await Mysqlhelper.insert(`
                insert into test(c_id,qg_id,score,t_type,start_time,end_time,work_time,test_name,is_random,demand) value(?,?,?,?,?,?,?,?,?,?)
                `, param.c_id, qg_id, questionData.allScore, param.t_type, param.start_time, param.end_time, param.work_time, param.test_name, 0, param.demand);
            //加入学生
            await testHelper.addStudentTest(res, param, false);
        }
        return res;
    ...
    static async deleteKnowledgeTest(u_id, c_id, test_id) {
        ...
        if (count > 0)
            await Mysqlhelper.delete(`delete from test where test_id=? and c_id=? and t_type=3`, test_id, c_id);
    }
