...
class testHelper {
    ...
    static get(target) {
        if (target < 1)
            target = 1;
        let num1 = [0, 0.1, 0.2, .3, .4, .5, .6, .7, .8, .9, 1];
        let num2 = [0, 0.1, 0.2, .3, .4, .5, .6, .7, .8, .9, 1];
        let num3 = [0, 0.1, 0.2, .3, .4, .5, .6, .7, .8, .9, 1];
        let num4 = [0, 0.1, 0.2, .3, .4, .5, .6, .7, .8, .9, 1];
        let num5 = [0, 0.1, 0.2, .3, .4, .5, .6, .7, .8, .9, 1];
        let arr = [];
        num1.forEach(item1 => {
            num2.forEach(item2 => {
                num3.forEach(item3 => {
                    num4.forEach(item4 => {
                        num5.forEach(item5 => {
                            if (item1 + item2 + item3 + item4 + item5 == 1) {
                                let num = 1 * item1 + 2 * item2 + 3 * item3 +
                                    4 * item4 + 5 * item5;
                                let sum = Math.abs(target - num);
                                if (sum == 0) {
                                    arr.push([item1, item2, item3, item4,
                                        item5, sum.toFixed(4)
                                    ]);
                                }
                            }
        ...
        if (arr.length == 0)
            throw ErrorHelper.Error400("无法产生计算难度占比");
        return arr.sort((a, b) =>
            a.filter(item => item == 0).length - b.filter(item => item == 0).length
        )[0];
    }
    ...
    static getQuestionTyping(questions) {
        ...
        questions.forEach(item => {
            if (data[range[item.q_type]][item.q_range] == undefined)
                data[range[item.q_type]][item.q_range] = [];
    ...
    static async saveToDb(c_id, test_name, qg_type, difficulty, questionData, sum, isRandom = 0) {
        let qg_id = await Mysqlhelper.insert(`
            ...
            `, c_id, test_name + "用题组", difficulty, qg_type, questionData.allScore, isRandom);
        for (let key in sum) {
            for (let i = 0; i < sum[key].length; i++) {
                await Mysqlhelper.insert(`
                    insert into group_question(qg_id,ql_id,score) value(?,?,?)
                    `, qg_id, sum[key][i].ql_id, questionData.question[key].score);
            }
        }
        return qg_id;
    }
    ...
    static async getQuestionGroup(questionData, persent, isExam) {
        let questionNum = parseInt(questionData.question.single.num) +
            parseInt(questionData.question.mult.num) +
            parseInt(questionData.question.gap.num) +
            parseInt(questionData.question.program.num);
        //查询选中章节下的题目
        let knowledgePont;
        ...
        let questions = await Mysqlhelper.row(`select DISTINCT a.ql_id,a.q_range,a.q_type from  question_knowledge b INNER 
    ...
    static async judgeQuestion(sc_id, test_id, u_id, c_id, qg_id, answer) {
        ...
        knowledgePointList.forEach(item => {
            if (questionKnowledge[item.ql_id] == undefined)
                questionKnowledge[item.ql_id] = [];
        ...
        for (let i = 0; i < questionList.length; i++) {
            ...
            if (question.q_type == '4')
                await testHelper.saveCode(u_id, test_id, question.ql_id, answer[question.ql_id]);
            allScore += getScore;
            let add = 0;
            if (getScore == question.score)
                add = 1;
            ...
            //更新知识点掌握情况
            if (questionKnowledge[question.ql_id] == undefined)
                continue;
        ...
        for (let key in knowledge) {
            try {
                ...
                `, knowledge[key].count, knowledge[key].rightCount, sc_id, key);
            } catch (error) {
                await Mysqlhelper.insert(`
                insert into student_knowledge(sc_id,kp_id,count,right_count) value(?,?,?,?)
                `, sc_id, key, knowledge[key].count, knowledge[key].rightCount);
            }
        }
        //更新学生能力值
        ...
        let evaluate = 0;
        for (let i = 1; i <= 5; i++) {
            if (studentData[`difficulty_${i}`] != 0)
                evaluate += i * (studentData[`difficulty_pass_${i}`] / studentData[`difficulty_${i}`]);
        }
        ...
        studentKnowledgeList.forEach(item => {
            if (item.count >= config.coverage.num && (item.right_count / item.count >= config.coverage.persent))
                coverage++;
        })
    ...
    static async judgeAnswer(realAnswer, answer, score, type, ql_id, sc_id, test_id) {
        ...
        if (type == 1) {
            ...
            for (let i = 0; i < tempAnswer.length; i++) {
                if (realAnswer.indexOf(tempAnswer[i]) != -1)
                    count++;
                else
                    return 0;
            }
            if (count == realAnswer.length)
                return score;
            else
                return score / 2;
        } else if (type == 3) {
            ...
            for (let i = 0; i < answer.length; i++) {
                if (answer[i].trim() == realAnswer[i].trim())
                    count++;
            }
            ...
            //编程题判分
            let form = {
                language_config: programConfig[answer[0]],
                max_cpu_time: 3000,
        ...
        try {
            for (let i = 0; i < param.choseClass.length; i++) {
                if (!is_random) {
                    ...
                    `, test_id, param.choseClass[i].id);
                } else {
                    await Mysqlhelper.insert(`
                    insert into student_test(sc_id,test_id) select a.sc_id,b.test_id from student_class a inner join test b on b.test_id=? where a.class_id=?
                    `, test_id, param.choseClass[i].id);
                }
            }
        } catch (err) {
    ...
    static async saveCode(u_id, test_id, ql_id, answer) {
        ...
        fs.writeFile(path.join(dir, u_id.toString()), answer[1]);
    }
