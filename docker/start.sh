...
docker run -itd --name=teach-system -v $path/../teachSystem:/opt/workspace --link redis --link mysql node:v1 /bin/bash /opt/start.sh

echo "login-system容器建立"
docker run -itd --name=login-system -v $path/../loginSystem:/opt/workspace --link redis --link mysql node:v1 /bin/bash /opt/start.sh
